...
export default {
      ...
      res.data.filter((item, index) => {
        ...
        if (-1 == _index) {
          _obj.push({
            ...
            data: [{ VehicleId: item["vehicleId"], name: item["vehicle"] }],
          });
        } else {
  ...
  async created() {
    ...
    this.carBrands.HotBrand = this.carBrands.Brand.filter(
      (item) => item.hot == "1"
    ).filter((item) => {
    ...
    let sortObj = _obj
      .slice(1)
      .sort((obj1, obj2) => obj1.letter.localeCompare(obj2.letter));
    _obj = [_obj[0], ...sortObj];
